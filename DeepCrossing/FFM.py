...
class FFM(BaseModel):
    ...
    def ffm_interaction(self, field_wise_emb_list):
        ...
        eg: 对应于v_ij
        1.取对应位置的emb layer
            i < j :取j-1 -> field_wise_emb_list[j - 1] :bs x F x d
            i > j ：取j -> field_wise_emb_list[j] :bs x F x d
        2.取出emb_layer里面第i个embedding向量，通过索引取
            field_wise_emb_list[j-1][:,i,:]
            field_wise_emb_list[j][:,i,:]
        '''
        dot = 0
        for i in range(self.num_fields - 1):
            for j in range(i + 1, self.num_fields):   # j>i
